...
if __name__ == "__main__":
    ...
    boc = BrainObservatoryCache(manifest_file='/network/projects/neuro-galaxy/data/raw/allen_brain_observatory_calcium/manifest.json')
    all_dg_exps = boc.get_ophys_experiments(stimuli=[stim_info.DRIFTING_GRATINGS])
    num_exps = len(all_dg_exps)

    ##CREATING NEW CSV FILES
    columns = ["exp_id", "subject_id", "cre_line",  "depth", 
                "num_seqs", "num_ROIs", "num_timepoints"]
    ...
    df.to_csv("/home/mila/x/xuejing.pan/POYO/project-kirby/kirby/AllenBOmeta.csv", index = False)
