...
logging_.basicConfig(level="INFO", format="%(message)s", datefmt="[%X]",
...
flags.DEFINE_enum('logging', 'info', ['debug', 'info', 'warn', 'error', 'critical'], 'Set logging level. (default: info)')
...
class logging:
    ...
    def init_logger(cls, level: str = 'info', rank: int = 0):
        ...
        logging_.getLogger().addHandler(RichHandler(rich_tracebacks=True, markup=True, omit_repeated_times=True, show_path=False, log_time_format='[%X]'))
...
set_level_all_loggers(logging_.WARNING)
